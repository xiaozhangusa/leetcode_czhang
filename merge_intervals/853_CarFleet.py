...
class Solution:
    def carFleet(self, target: int, position: List[int], speed: List[int]) -> int:
        # # 1. Pair position and speed and sort by position descending (closest to target first)
        # cars = sorted(zip(position, speed), reverse=True)
        
        # fleets = 0
        # lead_time = 0 # Time taken by the leader of the current fleet
        
        # for pos, spd in cars:
        #     time = (target - pos) / spd
            
        #     # If this car takes MORE time than the fleet leader in front,
